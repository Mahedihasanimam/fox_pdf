...
import { PrimaryButton, SecondaryButton, SectionHeader, ProgressBar, ScreenHeader } from '../components/UIComponents';
...
export default function ImageToPdfScreen({ navigation }) {
  ...
  const pickImages = async () => {
    ...
    if (!res.canceled) {
      ...
      setImages((prev) => [...prev, ...res.assets.map((a, i) => ({ ...a, key: `${Date.now()}-${i}` }))]);
    }
  };
  ...
  const renderDragItem = ({ item, drag, isActive }) => (
    <View style={[styles.imageItem, { backgroundColor: isActive ? colors.primary + '18' : colors.surface, borderColor: isActive ? colors.primary : colors.border }]}>
      <TouchableOpacity onLongPress={() => { Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Medium); drag(); }} delayLongPress={150}>
        <Ionicons name="reorder-three" size={22} color={colors.textLight} style={{ paddingHorizontal: 6 }} />
      </TouchableOpacity>
      <Image source={{ uri: item.uri }} style={styles.imageThumbnail} />
      <View style={styles.imageInfo}>
        <Text style={[styles.imageLabel, { color: colors.text }]} numberOfLines={1}>{item.fileName || 'Image'}</Text>
        <Text style={[styles.imageDims, { color: colors.textLight }]}>{item.width}×{item.height}</Text>
      </View>
      <TouchableOpacity onPress={() => removeImage(item.key)} style={[styles.removeBtn, { backgroundColor: '#FEE2E2' }]}>
    ...
    return (
      <SafeAreaView style={[styles.safe, { backgroundColor: colors.background }]}>
        <ScrollView contentContainerStyle={styles.resultContainer}>
          ...
          <Text style={[styles.resultTitle, { color: colors.text }]}>PDF Created!</Text>
          <Text style={[styles.resultSub, { color: colors.textSecondary }]}>Your images have been converted</Text>
          <View style={[styles.resultCard, { backgroundColor: colors.surface, borderColor: colors.border }]}>
            <ResultRow label="File" value={result.name} colors={colors} />
            <ResultRow label="Size" value={formatFileSize(result.size)} colors={colors} />
            <ResultRow label="Images" value={`${images.length} converted`} colors={colors} last />
          </View>
    ...
    return (
      <SafeAreaView style={[styles.safe, { backgroundColor: colors.background }]}>
        <View style={styles.processingContainer}>
          <Ionicons name="images" size={64} color={colors.imageToPdf?.icon || colors.primary} />
          <Text style={[styles.processingTitle, { color: colors.text }]}>Converting…</Text>
          <Text style={[styles.processingSubtitle, { color: colors.textSecondary }]}>Building your PDF from {images.length} images</Text>
          <View style={styles.progressWrap}>
            <ProgressBar progress={progress} color={colors.imageToPdf?.icon || colors.primary} />
  ...
  return (
    <SafeAreaView style={[styles.safe, { backgroundColor: colors.background }]}>
      ...
      <View style={{ flex: 1 }}>
        ...
        <TouchableOpacity style={[styles.pickZone, { backgroundColor: colors.imageToPdf?.bg || '#FFF3ED', borderColor: colors.imageToPdf?.icon || colors.primary }]} onPress={pickImages} activeOpacity={0.8}>
          <Ionicons name="images" size={44} color={colors.imageToPdf?.icon || colors.primary} />
          <Text style={[styles.pickZoneTitle, { color: colors.imageToPdf?.icon || colors.primary }]}>Select Images</Text>
          <Text style={[styles.pickZoneSubtitle, { color: colors.textSecondary }]}>JPG, PNG · Long-press to reorder · up to 20</Text>
        </TouchableOpacity>

        {images.length > 0 && (
          <View style={styles.listHeader}>
            <SectionHeader title={`Selected (${images.length})`} subtitle="Long-press handle to drag" />
          </View>
        )}
        ...
        <DraggableFlatList
          ...
          ListFooterComponent={
            images.length > 0 ? (
              <TouchableOpacity style={[styles.addMoreBtn, { borderColor: colors.imageToPdf?.icon || colors.primary }]} onPress={pickImages}>
                <Ionicons name="add-circle-outline" size={18} color={colors.imageToPdf?.icon || colors.primary} />
                <Text style={[styles.addMoreText, { color: colors.imageToPdf?.icon || colors.primary }]}>Add More Images</Text>
              </TouchableOpacity>
            ) : null
          }
        />
...
const styles = StyleSheet.create({
  ...
  errorBanner: { flexDirection: 'row', alignItems: 'center', gap: 8, borderRadius: RADIUS.md, padding: SPACING.md, margin: SPACING.base, borderWidth: 1 },
